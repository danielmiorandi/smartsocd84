...
\item {\bf GET /person\_sl?username=:username\&password=:password} Register a new human peer. Redirects to the newly created peer ID.  \todo{Ronald: should be a POST? -> Luc is right}
\end{itemize}
...
\item {\bf PUT /users/:id} Update user password: replaces the existing user password for the user with the given id. \todo{updates only pwd or user description?}
